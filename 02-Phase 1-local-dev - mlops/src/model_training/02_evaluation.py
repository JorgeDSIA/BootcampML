"""Etapa 2 de entrenamiento: evalúa el modelo entrenado contra test.csv.

Calcula accuracy/precision/recall/F1/ROC-AUC y grafica la importancia de
cada feature según los pesos aprendidos por la Regresión Logística.
"""
import pandas as pd
import numpy as np
import joblib
...
def evaluate_data(X_train, y_train, X_test, y_test):
    """Calcula métricas de evaluación y grafica la importancia de cada feature."""
    # load pipeline
    pipeline = joblib.load(MODEL_PATH)

    y_pred = pipeline.predict(X_test)  # predicciones sobre datos nunca vistos (test set)
    ...
    # feature importance: el peso (coeficiente) que la Regresión Logística le dio a cada feature
