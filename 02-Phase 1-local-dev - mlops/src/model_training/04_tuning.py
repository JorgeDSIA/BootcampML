"""Etapa 4 de entrenamiento: ajuste de hiperparámetros (tuning) con GridSearchCV.

Toma el pipeline ya entrenado, busca la mejor combinación de hiperparámetros
según Recall, y sobreescribe artifacts/model.pkl con el modelo ajustado.
"""
import json
import types
import pandas as pd
...
from config.paths import PREPROCESSED_TRAIN_PATH, PREPROCESSED_TEST_PATH, MODEL_PATH, METRICS_PATH, PIPELINE_PATH


def tuning_data(X_train, y_train, X_test, y_test):
    """Busca los mejores hiperparámetros, evalúa el modelo ajustado y lo guarda."""
    ...
    # Se guarda también el pipeline "crudo" (preprocesador + clasificador) para
    # explicabilidad (ver model_testing/explain.py): SHAP necesita ambos pasos por
    # separado, algo que el objeto envuelto de abajo ya no expone.
    joblib.dump(tuned_pipeline, PIPELINE_PATH)

    # Se envuelve el pipeline en un objeto simple cuyo .predict() en realidad llama a
    # predict_proba(): así el servicio de inferencia (predictor.py) siempre recibe
    # probabilidades [p_stay, p_leave] en vez de solo la clase 0/1.
