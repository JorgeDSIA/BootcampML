"""Etapa 2 del pipeline de datos: valida el esquema del dataset con Pandera.

Si una columna no cumple el tipo o los valores esperados, el proceso se detiene
aquí en vez de dejar pasar datos corruptos a las siguientes etapas.
"""
...
# Columnas esperadas: tipo de dato, valores permitidos y si acepta nulos
employee_schema = pa.DataFrameSchema(
    ...
    strict=False,   # permite columnas no listadas aquí (pasan sin validar)
    coerce=True     # intenta convertir los tipos de dato al tipo declarado
)


def validate_data(df):
    """Valida el DataFrame contra el schema; retorna None si encuentra errores."""
    try:
        validate_df = employee_schema(df, lazy=True)  # lazy=True: acumula todos los errores antes de fallar
